...
class SeqSet2SeqTransformer(nn.Module):
    ...
    def forward(self, src: Tensor, trg: Tensor, src_mask: Tensor,
        ...
        for j in range(src.shape[1]):
            src_emb = self.positional_encoding(self.src_tok_emb(src[i, j, :]))
            memory = self.transformer_encoder(src_emb, src_mask, src_padding_mask)
            src_transformed_list.append(memory)
        avg_src_transformed_embed = torch.mean(src_transformed_list)
        tgt_emb = self.positional_encoding(self.tgt_tok_emb(trg))
        outs = self.transformer_decoder(tgt_emb, avg_src_transformed_embed, tgt_mask, None,
